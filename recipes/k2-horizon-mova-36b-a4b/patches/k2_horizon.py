...
class K2HorizonMoVAAttention(nn.Module):
    def __init__(
        ...
        # Rig patch (K2-BLACKWELL): keep the MoVA value experts unquantized.
        # compute_mova_v_sparse() stacks the raw `expert.weight` tensors into a
        # bf16-only fused kernel (use_fp8_w8a8=False). With --quantization fp8
        # the Fp8LinearMethod transposes and casts those weights in place, and
        # the stacked tensor arrives as [E, hidden, kv/tp] in fp8, tripping
        # "Hidden size mismatch 2560 != 512". Upstream thread: the merge of
        # vllm-project/vllm#55063 has no quantized MoVA path yet. Retire when
        # fused_mova_impl accepts fp8 weights + scales.
